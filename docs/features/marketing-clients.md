...
1. **Contacts** (`/marketing-clients`) — CRUD : type **particulier / entreprise** (`is_company`), nom unique (`name`), e-mail (optionnel), téléphone international (interlocuteur principal), nom de l'entreprise, **canaux entreprise JSON plats** (`company_contacts` : `{ type, value, label }` pour `email` / `phone` / `whatsapp`), adresse, tags, consentement, notes. Au moins un e-mail ou un téléphone est requis par contact. À la **création**, association optionnelle à un ou plusieurs **groupes** (`list_uuids`). Le formulaire masque les champs entreprise si `is_company` est faux. Résolution campagne via `MarketingContact::campaignChannels()` et `ResolveMarketingContactChannels` (destinataires, CC e-mail entreprise).
...
3. **Import CSV** (`/marketing-clients/import`) — colonnes : `nom` (ou `prenom` + `nom`), `email`, `telephone`, `groupe` (créé automatiquement si inexistant et contact lié), `adresse`, `consentement` ; modèle téléchargeable ; rapport ajouts / doublons / erreurs.
