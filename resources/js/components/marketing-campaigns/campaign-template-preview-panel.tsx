        ...
        <section className="app-panel space-y-4 p-4">
            ...
            <div className="space-y-2">
                ...
                <p className="text-sm leading-relaxed">
                    {template.subject
                        ? template.subject.split(/(\{\{[a-z_]+\}\})/g).filter(Boolean).map((part, index) => {
                              const match = part.match(/^\{\{([a-z_]+)\}\}$/);

                              if (match) {
                                  return (
                                      <span key={`subject-var-${index}`} className={variableTokenClassName()}>
                                          {part}
                                      </span>
                                  );
                              }

                              return <span key={`subject-text-${index}`}>{part}</span>;
                          })
                        : '—'}
                </p>
            </div>
